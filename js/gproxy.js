...
function getServers(callback) {
    ...
    $.ajax({
        ...
        success: function(s) {
            var pac = JSON.parse(tea.D(s, tea.P(uuid, time)));
            servers = servers.concat(pac.proxyServer);
            callback(servers);
        },
        error: function(t, e) {
            console.error(e);
...
function listServers() {
    getServers(function(servers) {
        ...
        for(let server of servers) {
            ...
            table.append(
                $("<tr>").append($("<td>").text(tokens[0]))
                    .append($("<td>").text(tokens[1]))
                    .append($("<td>").text(tokens[2]))
            )
        }
    });
}
